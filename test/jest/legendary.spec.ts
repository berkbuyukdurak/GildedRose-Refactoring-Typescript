...
import { Legendary } from '@/inventory';
...
describe('Legendary Inventory', () => {
    ...
    it('should abide by the constructor limit', () => {
        const ragnaros = new Legendary('Sulfuras, Hand of Ragnaros', 80, -1);
        expect(ragnaros.sellIn).to.equal(0);
        expect(ragnaros.quality).to.equal(80);

        const helios = new Legendary('Sulfuras, Hand of Helios', 80, 100);
    ...
    it('should NOT degrade', () => {
        ...
        expect(ragnaros.sellIn).to.equal(0);
        expect(ragnaros.quality).to.equal(100);
    });
});